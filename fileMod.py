...
def storeFile(fName, fID, start, storageLoc, filePath):
    #Open Storage file
    with open(storageLoc, 'rb+') as storage, open(os.path.join(filePath, fName), 'rb') as inputFile:
...
def retFile(fileMD, storageLoc, fileToPut):
    if fileMD == -1:
