    ...
    while True:
        try:
            ...
            try:
                ...
                print(repr(e))
            print("----Server did not respond reconnecting-----")
            if ctr == 5:
                client.close()
...
def do_SFTP(host, port, serverName, password):
...
def main():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client:
        ...
        if config["Registered"]:
            print('registered Procedding to hearbeat')
            p1 = multiprocessing.Process(
                target=Heartbeat, args=(client, config,))
            p1.start()

            p2 = multiprocessing.Process(target=do_SFTP, args=(
